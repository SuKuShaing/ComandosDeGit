...
git commit -am "Mensaje"        # es como git add + git commit, pero solo funciona si el archivo ya fue agregado al repositorio antes
...
git diff <a> <b>                # Muestra las diferencias entre dos commits, ramas o archivos
...
git tag <nombre-tag>            # Crea una etiqueta (tag) en el commit actual
